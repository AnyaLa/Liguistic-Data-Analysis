...
russian <- read.csv("http://goo.gl/KljqjU", sep=",", encoding = 'UTF-8') # read using URL for the file
```
...
```{r 1_6}
sqrt(1.6459/((156 + 96 + 119 + 95)*(min(2,2) - 1)))
#Cohen book on statistics for interpretation
#0.05 - very small result for 1 degree of freedom
```
...
```
```{r}
chisq.test(polish.tab)$expected
```
